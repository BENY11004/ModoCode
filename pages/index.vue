  ...
  if (window.innerWidth < 700) {
    ...
    if (sideBar.style.display === "flex") {
      setTimeout(() => {
        ...
        sideBar.classList.add("de-active");
      }, 10);
      ...
      setTimeout(() => {
        sideBar.classList.remove("de-active");
...
let loadProjects = async () => {
  ...
  for (let i = 0; i < projects.results.length; i++) {
    ...
    setTimeout(() => {
      let elementId = `project-${project.id}`;
      let projectElement = document.getElementById(elementId);

      if (projectElement) {
        projectElement.addEventListener("click", () => {
          history.pushState(null, null, `?id=${project.id}`);
...
function sendPrompt() {
  ...
  let prompt = document.getElementById("message-box").value;
  ...
  if (query.has("id")) {
    if (prompt != "") {
      ...
      chatBox.innerHTML += `<div id="message" class="bg-[rgb(203,184,247)] text-right min-h-16 flex items-center pb-1 px-4 rounded-xl relative mt-3 min-w-16 shrink-0"><div class="mb-4">${prompt}</div><div class="absolute bottom-0.5 right-4">${new Date().getHours().toString().padStart(2, "0")}:${new Date().getMinutes().toString().padStart(2, "0")}</div></div><div id="wait-animation" class="Animation my-3"></div>`;
      ...
      if (new URLSearchParams(location.search).get("id") != "") {
        askQuestionApi(query.get("id"), prompt)
          .then(res => {
            let markdownText = res.data.ai;
            let answer = marked.parse(markdownText);

            chatBox.innerHTML += `<div id="message" class="bg-[rgb(228,216,255)] text-right min-h-16 flex items-center pb-1 px-4 rounded-xl relative mt-3 min-w-16 shrink-0"><div class="mb-6">${answer}</div><div class="absolute bottom-0.5 right-4 ">${new Date().getHours().toString().padStart(2, "0")}:${new Date().getMinutes().toString().padStart(2, "0")}</div></div>`;
            document.getElementById("chat-box").scrollTop = document.getElementById("chat-box").scrollHeight;
        ...
        messageBox.placeholder = "please select a project!";
        messageBox.classList.add("select-project-error");
        setTimeout(() => {
          messageBox.placeholder = "type your prompt here";
          messageBox.classList.remove("select-project-error");
        }, 5000);
      }
    }

    let message = document.querySelectorAll("#message");
    message.forEach(prompt => {
      prompt.classList.add("animate-up");
    });
  } else {
    alert("لطفا یک پروژه را انتخاب کنید");
    messageBox.placeholder = "please select a project!";
...
<template>
  <div class="flex lg:w-full min-h-screen overflow-hidden relative">
    <div v-if="isSidebarOpen" class="lg:w-3/12 w-9/12 h-screen z-50 lg:flex flex-col items-center bg-black relative px-8" id="side-bar">
        ...
        <div>
          ...
          <div class="mx-4">
            <div class="w-full h-10 bg-[#424242] rounded-xl mt-8 flex items-center px-4">
              ...
              <input class="text-white ml-2 text-sm font-semibold outline-none w-full" placeholder="Search" />
            </div>
      ...
      <div class="z-50 h-14 lg:w-full w-11/12 bg-[#242723] rounded-xl flex items-center px-4 justify-between">
        ...
        <img src="/img/logout.svg" class="size-3 cursor-pointer hover:opacity-70" title="Log out" />
      </div>
    </div>
    <div class="lg:w-9/12 w-full flex flex-col justify-between" id="main-screen">
      <div>
        <div class="w-full flex justify-between items-center">
          ...
          <div class="flex justify-center w-full">
            <div class="w-32 h-10 bg-[rgb(217,205,245)] mt-11 mr-[15%] rounded-full flex items-center justify-center mb-1"><p id="project-name"></p></div>
          </div>
          <div class="flex items-center justify-center flex-col absolute top-[45px] lg:right-[80px] right-[50px] lg:size-[60px] size-[40px] z-50 hover:bg-black/10 rounded-full">
            <div class="flex items-end flex-col" @click="isSidebarOpen =  !isSidebarOpen">
              <div class="w-[30px] border-b-[3px] bg-black mt-2 rounded-full"></div>
              <div class="w-[20px] border-b-[3px] bg-black mt-1 rounded-full"></div>
            </div>
          </div>
        </div>
      ...
      <div class="flex flex-col justify-center w-full lg:px-12 px-5 py-4 z-40">
        <div dir="ltr" id="chat-box" class="max-w-8/12 absolute bottom-28 right-[50px] lg:max-h-[500px] max-h-[400px] h-fit flex-col overflow-y-auto hidden"></div>
        <div id="information-text" class="flex flex-col items-center w-full z-20">
          <div class="text-center lg:w-[445px] w-[330px]">
            ...
            <p class="lg:text-[16px] text-sm">
            </p>
          </div>
        </div>
      </div>
      <div class="flex justify-center items-center lg:mb-8 mb-24 lg:z-50 z-20">
        <div class="w-11/12 h-16 bg-[#8C8C8C3D]/75 rounded-full flex justify-between px-2 items-center">
          <div class="flex items-center">
            <div>
              <button class="bg-[#424242] size-13 rounded-full flex items-center justify-center cursor-pointer hover:opacity-80">
                <img src="/img/paperclip.svg" class="size-7" />
              </button>
            </div>
          </div>
          <input @keypress.enter.exact="sendPrompt" id="message-box" placeholder="type your prompt here" type="text" class="placeholder:text-black w-full mx-5 outline-none h-10" />
          <div class="flex items-center justify-center rounded-full" @click="sendPrompt()">
            <button class="bg-[#9747FF] size-13 rounded-full flex items-center justify-center cursor-pointer hover:opacity-90">
